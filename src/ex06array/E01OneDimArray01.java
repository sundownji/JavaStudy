...
public class E01OneDimArray01 {
	...
	/*
	배열: 순차적인 메모리의 공간이 할당되는 형태로써 하나가 아니라 여러개의 변수가 필요한
	경우 사용한다.
	...
	*/
	public static void main(String[] args) {
		...
		//<배열명= 이상한 형태의 문자로 표현되는데 이게 할당된 주소값임>
		System.out.println("배열명(numArr)="+numArr);
		System.out.println("===========================");
		
		/*
		배열의 선언 및 초기화:배열은 여러개의 변수를 사용하기 위해 선언하므로 접근을 위해
		일반적으로 반복문(for)을 사용한다.
		<40->400->4000->.. 반복문이 있기 때문에 여러 번 반복 가넝, while 사용 가능
		이지만 많이 불편함> 
		 */
		...
		*/
		...
		/*배열의 선언과 동시에 초기화하는 경우 자바 컴파일러는 초기값의 갯수를 통해 배열의 크기를
		결정할 수 있다. 따라서 이런 경우에는 크기는 생략하도록 약속되어있다. */
		System.out.println("방법2: 선언과 동시에 초기화1");
//		int[] initArr2 = new int[3] {1,20,300}; //에러 발생.
		int[] initArray2 = new int[] {1,20,3000};
		
		/*배열 원소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에 진행한다. 이 경우
		new를 생략할 수 있다.*/
		System.out.println("방법3: 초기값만으로 선언");
		//원소의 갯수를 통해 크기 5로 지정된다.
		int[] initArr3 = {11,13,15,17,19};
		int arrSum = 0;
