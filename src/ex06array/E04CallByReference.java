...
메서드를 호출할 때 참조값을 전달함으로써 서로 다른 지역에서도 동일한 메모리(힙영역)를
참조할 수 있게 하는 호출 방식이다. 메서드의 지역이 다르더라고 참조값을 통해 객체를 참조하게
되므로 A지역에서의 값의 변경이 B지역에서도 그대로 참조할 수 있는 장점이있다.
...
*/
	...
	static void callByReference(int[] ref) {
		/*
		매개변수를 통해 전달받은 배열객체의 참조값을 통해 0번 원소와 1번 원소를
		서로 교환한다. 이때 배열의 값이 서로 바뀌게된다.
		*/
		int temp;
		temp = ref[0];
		ref[0] = ref[1];
