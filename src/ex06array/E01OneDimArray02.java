...
public class E01OneDimArray02 {
	...
	public static void main(String[] args) {
		/*
		난수 생성 방법1: Math클래스의 random() 메서드는 0~1사이의 실수를 랜덤하게 반환한다. 
		정수형태의 난수가 필요하면 10,100과 같은 정수를 곱해주면 된다. 
		*/
		double rndNumber = Math.random();
		System.out.println("생성된난수[실수]:"+rndNumber);
		//정수와 실수를 연산하면 실수가 되기 때문에 (int)를 앞에 붙여줘서 강제형변화를 한다.
		int intNumber = (int)(Math.random()*100);
		System.out.println("생성된난수[정수]:"+intNumber);
		
		/*
		난수 생성 방법2: Random 클래스를 사용한다. 사용법은 Scanner와 동일하게
		...
		*/
		...
		//2.45로 나누기 연산하여 나머지를 구한다.정수를 45로 나누면 나머지는 0~44사이가 된다.
		//3.0은 구간에 포함되지 않으므로 1을 더해준다.
		//4.정수의 결과를 구하기 위해 int로 강제형변화해준다.
		...
		*/
		System.out.println("크키가 6인 배열에 난수 입력");
		//크기가 6인 정수형 배열 생성
