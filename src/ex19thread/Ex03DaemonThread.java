...
일컫은 것으로 메인쓰레드가 종료되어도 계속 실행되며, Dead상태가 되어야만 실행이 종료되는
쓰레드를 말한다. 해당 쓰레드는 main() 메서드가 종료된 이후에도 계속 실행되는 것을 볼 수 있다.
*/
class NormalThread extends Thread{
	//기본생성자를 통해 인스턴스를 생성하면 쓰레드명이 JVM에 의해 자동으로 부여
	public NormalThread() {}
	/*Thread클래스의 생성자 호출시 전달하는 인수를 통해 해당 쓰레드의 이름을
	지정할 수 있다. 이때 지정된 이름은 getName()을 통해 반환 받을 수 있다. */
	public NormalThread(String title) {
		super(title);
	}
	/*
	쓰레드 동작중 비정상적으로 종료될 때 InterruptedException이 발생할 수 있으므로,
	예외처리를 해야한다. 여기서는 즉시 처리하지않고, throws해서 호출한 지점으로
	예외객체를 던지기한다.
	*/
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		 *쓰레드로 구현하지 않은 일반적인 클래스의 작업으로 순차적으로 진행되어
		 *1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다.
		 (*첫번째는 new 인스턴스생성 후 참조변수로 클래스메소드 호출
		 *두번째는 생성과 동시에 호출)*/
		...
		//쓰레드명을 지정
		dt.setName("데몬쓰레드");
		//setDaemon의 값을 true로 주면 데몬쓰레드가 된다.
		//setDaemon없애면 독립 Thread가 되서 계속 돌아감
		dt.setDaemon(true);
		dt.start();
