...
/*Thread(쓰레드)
 ...
 -만약 해당 클래스가 다른 클래스를 상속해야한다면 Runnable 인터페이스를 구현하여 사용한다.
 다중상속 안된디.*/
		...
		for(int i=1; i<=100; i++) {
			...
			try {
				/*
				 ...
				 주어진 시간만큼 block상태로 전환시켜준다. 1000분의 1초 단위로
				 기술하게 되므로 아래는 0.01초만큼 block 시키게된다. 
				 */
				sleep(10);
			}
