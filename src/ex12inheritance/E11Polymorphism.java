...
/*다형성(Polymorphism)
 : 하나의 인스턴스(객체)를 여러 타입의 참조변수를 통해 참조할 때 참조변수의 타입에 따라
 다양한 형태의 인스턴스를 이용할 수 있는 특성을 말한다.
 
 -부모타입의 참조변수로 자식인스턴스를 참조
 1.이 경우 부모타입의 참조변수는 부모로부터 상속받는 멤버까지만 접근할 수 있다.
...
class Parent{
	...
	//	@Override //부모 클래스 위에 암것도 없는 줄 알았지만 사실 object 클래스 밑에 있는 거라 가능한
...
class Child extends Parent{
	...
	/* 메서드명은 동일하지만 매개변수가 틀리므로 오버로딩으로 정의된 메서드이다. 
	   따라서 자식에서 확장한 멤버가 된다. */
	//오버로딩됐음 (매개변수 1개 받고 있)
...
public class E11Polymorphism {
	...
	homeChild.parentMethod(1000);//오버로딩한 메서드(매개변수 갖고 있는)
	homeChild.parentMethod();//오버라이딩한 메서드
	
	/* 헤테로지니어스 이질화: 부모타입의 변수로 자식인스턴스를 참조하는 것을 말한다. 
...
//	parent1.childMember = 1; -자식에서 새롭게 정의된 멤버변수는 부모가 접근하지 못함
	...
	/* 부모 참조변수로 접급하려면 강제형변환 해야한다. 하지만 아래와 같이 기술하면 에러가 발생한다.
	   (Child)parent2.childMember
	   소괄호를 하나 더 추가해서 parent2를 자식타입으로 형변환 후 멤버변수에 접근해야 한다. */
	...
	/*Java에서 생성한 모든 클래스는 Object 클래스를 상속한다. 따라서 모든 인스턴스는
	  Object의 참조변수로 참조가 가능하다. 또한 Object클래스에 정의된 모든 메서드를 별도의
	  선언없이 사용할 수 있고, 필요하다면 오버라이딩도 할 수 있다.*/
