...
/*오버라이딩(Overriding)
 ...
 오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히 똑같은 형태로
 정의하는 것이다.
...
class Speaker{
	...
	//멤버변수가 private이므로 외부에서 설정할 수 있도록 setter를 정의함.
...
class BaseSpeaker extends Speaker{
	...
	public void showState() {
		/* super를 통해 부모클래스에 정의된 멤버메서드를 호출한다.
		  특히, 오버라이딩 된 메서드를 호출할 때 super가 없으면 재귀호출이
		  되므로 주의해야한다. */
		super.showState();
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		/* 오버라이딩 된 메서드를 호출하면 자식쪽의 showState()가 호출된다. 부모쪽의
		   메서드는 자식에 의해 가려지게 된다.*/
		baseSpeaker.showState(); //볼륨과 베이스가 출력
		...
		/* 단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는 범위가
		   부모 인스턴스까지로 제한되므로, 자식쪽에 정의되어 있는 setBase()는
		   호출할 수 없게 된다.*/
		speaker.setVolumn(30);
//		speaker.setBase(40); 
		/* 접근범위는 앞에서 설명한 바와 같이 부모까지로 제한되지만 오버라이딩 된
		 * 메서드가 있다면 자식쪽의 메서드가 호출된다.*/
		speaker.showState();
	}
