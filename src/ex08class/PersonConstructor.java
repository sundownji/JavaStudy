...
/*
 ...
 -반환값이 없다. 따라서 선언시 void를 사용하면 아예 에러가 발생한다. //또 다른 함수가 될 수있음 
...
public class PersonConstructor {
	...
	/*디폴트 생성자 
	 : 해당 클래스에 생성자를 정의하지 않으면 아래와 같이 매개변수, 실행부가 없는
	 	생성자가 컴파일러에 의해 자동으로 삽입된다. 지금까지 우리가 new 클래스명()과
	 	같이 사용했던 문장이 바로 디폴트 생성자를 호출하여 객체를 생성하는 것이었다.*/
	//클래스는 무조건 생성자가 있다.
	//****** 클래스 - 멤버변수, 멤버메서드, 생성자 ********
	...
	void initialize(String name, int age, String addr) {
		//위에 멤버메서드는 개발자가 맘만먹으면 만들 수 있음
		/*해당 코드는 생성자 내에서만 사용할 수 있다. 생성자는 인스턴스 생성시 딱
		한번만 호출되고, 개발자가 임의로 호출할 수 없다. 멤버메서드는 개발자가 원할때
		언제든 호출이 가능하므로 생성자와는 용도와 특성이 다르다. 따라서 멤버메서드에서는
		생성자를 호출 할 수 없다. */ 
