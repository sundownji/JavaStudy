...
-행동의 조건
 ...
 무한히 먹을 수 없고, 에너지는 마이너스가 될수없기 때문
*/
public class Human {
	//멤버변수: 객체의 속성을 표현한다.
	String name;
	...
	//멤버메서드: 객체의 행위를 표현한다.
	void walking() { //걷는다를 표현
		energy--;
		/* 걷는 행위는 에너지를 1 차감시킨다. 만약 에너지를 차감한 결과가 음수라면 해당 시나리오의 조건에
		 위배되므로 0으로 고정하는 코드가 포함되어있다. */
		if(energy<0){
	...
	//생각하다를 표현
	void thinking() {
		//생각하는 행위는 에너지를 2 차감시킨다. 차감 이후 음수라면 조건에 위배되므로 0으로 고정하는 코드가 포함.
		energy -= 2;
		if(energy<0) {
	...
	//먹는다를 표현
	void eating() {
		//먹는다는 행위는 에너지가 2 증가시킨다. 증가 후 10을 초과하면 10으로 고정한다.
		energy += 2;
		if(energy>10) {
			energy = 10;
