...
class ChildProperty{
	...
	public ChildProperty(int bead) {
		...
		/*
		 this의 두가지 활용법
		 1.생성자에서 또 다른 생성자 호출(생성자 오버로딩(개수 타입을 다르게 같은 이름의 메소드 작성 가능))
		 	=> this(인수1, 인수2)
		 2.멤버변수와 매개변수를 구분할 때 사용
		 	=> this.멤버변수명 = 매개변수명
		 */
	}
	//멤버메서드
	...
	void obtainBead(ChildProperty child, int beadCnt) {
		/* 해당 멤버메서드를 호출하는 어린이 객체는 게임에서 승리하여 구슬을
		 획득해야하고, 매개변수로 전달되는 어린이 객체는 패배하여 구슬을 상실해야한다. */
		//패배한 어린이 인스턴스는 구슬을 차감한다.
		child.bead -= beadCnt;
		//승리한 어린이(객체자신)은 구슬을 증가시킨다.
		this.bead += beadCnt;
	}
}
public class QuMarbles {
	...
	public static void main(String[] args) {
		...
		//주체의 객체가 된 애가 이긴거라 위에 매개변수로 전달된 child는 진 친구
