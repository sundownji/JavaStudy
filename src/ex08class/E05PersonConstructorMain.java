...
public class E05PersonConstructorMain {
	...
	public static void main(String[] args) {
		...
		/*인스턴스 생성시 초기화 메서드를 사용하면 생성과 초기화를 별도로 진행해야한다. 
		 하지만 생성자를 사용하면 생성과 초기화를 동시에 진행할 수 있어 편리하다.*/
		//생성자 정의, 생성자 객체를 생성함과 동시에 초기화하는게 좋은 방법임
		
		//기존에 생성했던 person1을 초기화 메서드를 통해 값을 변경한다.
		System.out.println("[멤버변수 초기화용 메소드 호출후]");
		person1.initialize("홍길동", 20, "논현동");
		person1.showPersonInfo();
		
		//오버로딩으로 정의된 각각의 생성자를 통해 인스턴스 생성 및 초기화를 한번에 진행한다.
