...
-정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에서 초기화된다.
...
-클래스 외부: 
	1.인스턴스 변수를 통해 접근한다.(권장사항이 아님:코드로 보기에 두개가 달라보이기 때문임)
	2.인스턴스 생성없이 클래스명으로 직접 접근한다. (권장사항)
	
-변수,메서드 앞에 static 붙이면 정적변수, 정적메서드가 되며 모든 특성이 동일하다.
...
class MyStatic{
	/*	멤버변수: 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 참조변수를 통해
	 	접근해야한다.
	 	즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모리에 로드된다.(개발자가 코드를 통해)
	 	<ex) Mystatic a = new Mystatic(); new 사용할때 
	 	(멤버변수는 Stack 영역, 인스턴스는 heap 영역에 생성)*/
	int instanceVar;
	...
	/*정적멤버메서드: 정적메서드 내부에서는 인스턴스형 멤버엔 접근할 수 없다.
	 	정적 멤버만 접근할 수 있다.(main()에서는 static으로 선언된 메서드만 
	 	호출할 수 있었음을 기억해라.) */
	static void staticMethod() {
		//static 일반 변수에 접근 안됨
		//static 일반 메서드 접근 안됨
		//static 지들끼리만 불러올 수 있음
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*정적변수도 일반적인 멤버변수처럼 참조변수를 통해 접근할 수 있지만,
		 Java에서는 권장하지 않는다.(참조변수를 통해 접근할거면, 
		 굳이 정적변수를 사용할 이유가 없기 때문이다.)*/
		myStatic.staticVar=300;
		System.out.println("mystatic.staticVar="+myStatic.staticVar);
	
		////////////////////////////////////////////////////////////
		/*2개의 인스턴스 생성한다. 메모리(Heap)에는 ms1,ms2의 인스턴스가 각각 생성된다.*/
		...
		ms2.staticVar = 900;	
		//아래에서는 둘 다 900이 출력된다. 
		//나중에 넣는게 원래 있던 걸 삭제하고 들어가는거라 900이 맞음
		...
		/*
		 클래스명을 통한 접근은 정적메서드만 가능하다. 인스턴스메셔드는
		 인스턴스 참조변수를 통해서만 접근할 수 있다.*/
		//MyStatic.instanceMethod(); //에러발생
		MyStatic.staticMethod(); //권장사항
	}
	/*결론 : 일반적인 인스턴스형 멤버는 객체의 참조변수를 통해 접근한다.
			정적 멤버는 클래스명을 통해 인스턴스 생성없이 접근한다. */
