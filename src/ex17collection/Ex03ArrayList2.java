...
import common.Student; //패키기가 달라서 import 사용
...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		  ...
		  즉, Student인스턴스를 저장할 수 있는 상자가 된다.
		*/
		LinkedList<Student> list2 = new LinkedList<Student>();
		//ArrayList<Student> list2 = new ArrayList<Student>();
		
		//저장할 Student인스턴스 생성, 외부패키지에 선언되었으므로 반드시 import후 사용
		...
		//일반 for문 사용을 위해 size()메서드로 추가된 인스턴스의 갯수를 반환받는다.
		//또한 index로 접근해야하므로 get() 메서드를 사용한다.
		for(int i = 0; i<list2.size(); i++) {
			//List의 각 인덱스에 저장된 인스턴스의 참조값을 그대로 출력한다.
			//toString() 메서드를 오버라이딩 했으므로 객체에 저장된 멤버변수값이 출력된다.
			//만약 오버라이딩이 되어있지 않다면 참조값(주소)이 출력될 것이다.
			System.out.println(list2.get(i));
		}
		
		//st2(참조값)똑같은 거 두개면 앞에 먼저 삭제된다.
		...
		/*
		시나리오] 컬렉션에 저장된 객체를 이름으로 검색하여 삭제할 수 있는 코드를 작성
		이때 반복문은 확장 for문을 사용한다. */
		int idx = 0;
		...
		for(Student st: list2) {
			/* 이름으로 검색시 멤버변수 name은 private으로 선언되었으므로,
			  getter를 통해 접근해야한다. 삭제를 위해 입력한 이름과 Student객체에
			  저장된 name이 일치하는지 확인한 후 해당 인스턴스를 찾을 수 있다.*/
