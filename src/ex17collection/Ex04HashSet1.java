...
HashSet<E> : set계열의 컬렉션
 ...
 -하지만 개발자가 정의한 클래스라면 hashCode(), equals()메서드를 적절히 오버라이딩
	해야만 중복을 제거할 수 있다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/* 기본클래스의 객체 중복저장: 기본 클래스인 경우 별도의 오버라이딩 없이도
		  중복이 제거된다. 따라서 아래의 add()메서드는 false를 반환한다. */
		...
		*/
		...
		while(itr.hasNext()) {
			...
			/*HashSet<E>에 저장된 모두 Object형으로 자동형변환되어
			...
			*/
			System.out.println(set.contains(varString1) ? "varString1 있음" : "varString2 없음");
			
			/*
			객체삭제: set컬렉션은 인덱스가 없으므로 객체의 참조값을 통해서만 삭제할 수 있다.
			*/
