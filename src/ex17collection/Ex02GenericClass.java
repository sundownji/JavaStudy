...
/*기존 Object기반의 FruitBox를 아래와 같이 제네릭 클래스로 변경
자료형형에 해당하는 부분을 타입매개변수로 교체하고 
인스턴스 생성시 자료형을 결정하기 위해 클래스명 <T>형태로 변경 */
...
public class Ex02GenericClass {
	...
	public static void main(String[] args) {
		/* Box인스턴스를 생성할 때 T부분을 결정하므로 구현의 편의성이 보장된다.
		...
		/*OrangeBox는 인스턴스 생성시 Orange를 저장할 수 있는 용도로 생성되었기 때문에
		  다른 객체는 아예 저장할 수 없다. 아래 코드에서 컴파일 에러가 발생하므로 자료형에도
		  안전한 코드가 된다.*/
//		orangeBox.store("나는 오렌지"); //컴파일에러 String 넣어서 안돼
//		orangeBox.store(apple); //오렌지박스에 사과 넣어서 안돼
//		orangeBox.store(banana); //오렌지박스에 바나나 넣어서 안돼
