...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		/*String인스턴스를 저장할 수 있는 List컬렉션 생성 ArrayList와 나머지
		  컬렉션들은 데이터를 저장하는 내부적인 자료구조만 다를뿐 사용법은 100% 동일하다.*/
...
//		list.add(5,"소방차"); //에러발생됨
		...
		*/
		...
		/*중복저장: list는 배열의 특성을 가지므로 데이털르 중복저장 할 수 있다.
		 동일한 데이터라도 index로 구분할 수 있기 때문이다. 
		 add()는 추가에 성공할 경우 true를 반환한다.*/
		//삼항연산자 사용 ? true:false
		System.out.println(list.add("트와이스") ? "중복저장됨":"중복"
				+ "저장안됨");
		System.out.println("중복 저장후 객체수:"+ list.size());
		
		/*
		  컬레션 출력시에는 아래 3가지 방법을 사용한다.
		  방법 1 : 일반 for문 사용. 이때는 인덱스를 통해 접근해야 하므로 get()메서드 사용
		 */
		System.out.println("\n[일반for문 사용]");
		for(int i= 0; i<list.size(); i++) {
			System.out.print(list.get(i)+ " " );
		}
		//방법2: 확장 for문. 인덱스 없이 전체원소를 반복해서 좀 더 간단히 표현, 가장 많이 사용
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		Iterator 사용법
		방법3: Iterator 사용
		 	1.컬렉션에 저장된 내용을 Iterator 객체에게 알려주기위해 인스턴스를 생성한다.
		 	2.hasNext()로 출력할 인스턴스가 있는지 검사한다. 만약 인스턴스가 더 이상 없다면
		 	false를 반환한다.
		 	3.앞에서 확인 후 next()를 통해 인스턴스를 출력한다.
		*/
		System.out.println("\n[반복자(Iterator) 사용]");
		Iterator<String> itr = list.iterator();
		//Iterator itr -> String없어도 가능 대신 그자리에 Object가 있다고 생각하면 된다.
		while(itr.hasNext()) {
			System.out.print(itr.next()+" ");
		}
		System.out.println("\n=====================\n");
		
		/*덮어쓰기(수정하기):기존의 내용 변경. 즉 기존의 내용은 삭제한 후 새로운 내용 갱신*/
		...
		*/
		...
		/*포함여부확인
		 	contains(객체):특정 인스턴스가 저장되어있는지 확인할 때 사용한다.
		 		존재한다면 true를 반환한다.
		*/
		 ...
		 *remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 
		  ...
		  삭제성공시 해당 인스턴스를 반환하므로 삭제 후 즉시 할인할 수 있다.*/
		Object obj = list.remove(2);
		//2번 인덱스 삭제 - 트와이스 반환
		System.out.println("삭제된 객체:"+ obj);
		
		/*
		삭제 방법2: (String indexOf를 활용 찾아서 지워주는거임)
		...
		*/
		...
		/*전체 삭제: 아래 2개의 메서드를 통해 컬렉션에 저장된 전체 데이터를 삭제할 수 있다.*/
		list.removeAll(list);
		list.clear();
