...
CPU가 메모리의 생성 및 소멸을 주관한다.*/	
	...
	public static void main(String[] args) {
		...
		boolean scope =true;
		/*
		메인지역에서 num을 선언했기 때문에 그보다 좁은 지역인 if문에서 같은 이름의
		변수를 선언할 수 없다. 큰 지역에서 이미 선언되어 메모리에 상주하고 있기 때문이다. 
		*/
		
//		int num = 9; //메인 지역에서 num을 선언했기 때문에 아래 코드에서 에러가 발생
		...
		*/
		if(scope) {
			...
			int num =5;
			System.out.println("첫번째 else문 지역:"+num);
		}
		/*
	...
	/*simpleFunc()는 main에서 선언한 변수 num은 메모리에
	아직 상주하고 있지만 해당 메서드는 크기가 동일한 다른 지역에
	정의되었으므로 같은 이름의 변수를 선언할 수 있다.
