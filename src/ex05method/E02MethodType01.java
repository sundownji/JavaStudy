...
규칙	 - 메서드는 반드시 class 내부에서 정의해야 한다.
 	 - 메서드 내부에 또 다른 메서드를 정의할 수 없다. 이런 경우 에러가 발생
 	 - 반환값이 없다면 void를 반드시 명시해야 한다.
 	 - 함수명은 변수명과 동일한 규칙으로 작성한다.

Java에서는 Naming Rule(이름 지을때 규칙)<가독성이 높아져서 규칙이 있음>
-클래스명 : SimpleFunc 
	=> 대문자로 시작하는 Camel case로 작성한다.
-메서드 혹은 변수명 : simpleFunc-변수 /simpleFunc()-함수
	=> 소문자로 시작하는 Camel case로 사용한다.
-상수명 : SIMPLE_FUNC 
...
public class E02MethodType01 {
	...
	: 둘 다 없는 형태의 메서드로 주로 메뉴를 출력하는 단순 출력 기능 정도로 사용 */
	...
	static void returnError() {
		...
		/*
		이와 같이 메서드 중간에 return을 기술하면 즉시 종료되므로 그 아래에 있는 코드는
		실행되지 않는 쓰레기 코드가 된다. 따라서 return을 사용할 때는 반드시 조건문과 같이
		기술되어야 한다. 단 메서드의 마지막 라인에 기술한다면 조건문이 없어도 된다. 
		*/
