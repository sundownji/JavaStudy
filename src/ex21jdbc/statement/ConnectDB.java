...
/* 상속을 목적으로 생성하는 추상클래스. 
오라클 DB연결과 자원반납을 담당한다.
추상클래스는 객체를 생성할수는 없지만 참조변수선언, 상속등의 기능은 사용할 수 있다.*/
public abstract class ConnectDB {
	
	//멤버변수 : 상속관계에서 접근가능한 접근지정자로 지정함.
	protected Connection con; //DB 연결을 위한 객체
	...
	/*생성자2 : 앞에서 선언한 생성자를 복사하여 선언된 생성자로 고정된 값으로 DB연결을 진행한다.
	오버로딩으로 정의되었다. */
	public ConnectDB() {
		try {
			//드라이버 로드
	...
	public void close() {
			try {
				/* Statement, Connection, ResultSet 객체에 대한
				 자원반납 처리*/
	...
	/*
	상속관계에서 오버라이딩을 목적으로 생성한 추상메서드로 하위클래스에서 각 업무(update, select 등)
	에 따라 재정의하게 될 것이다.
