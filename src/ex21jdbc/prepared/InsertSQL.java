...
public class InsertSQL extends IConnectImpl{
	...
	public void execute() {
		try {
			/*
			1.쿼리문준비 : 값의 세팅을 위한 부분을 ?(인파라미터)로 기술한다.
			*/
			String query = "INSERT INTO member VALUES(?,?,?,?)";
			
			/*2.prepareStatement객체 생성: 준비한 쿼리문을 인수로 전달한다.*/
			...
			/*
			4.인파라미터설정 : ?의 순서대로 설정하고, 인덱스는 1부터 시작하면된다.
				자료형이 
					숫자면 setInt()
					문자면 setString()
					날짜면 setDate()
			...
			*/
			...
			psmt.setString(4, "2023-07-31");
			
			//날짜입력2 : Date형으로 입력하는 경우
			/*현재날짜를 Java에서 입력하는 경우 아래와 같이 변환과정을 거쳐야한다.
			 util패키지ㅣ로 시간을 얻어온 후 sql패키지로 타입을 변환한다.
